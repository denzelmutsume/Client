...
function UIMinimap:onCameraPositionChange(cameraPos)
  ...
  local map = self:recursiveGetChildById('mapId')

	if not map then
		map = g_ui.createWidget('MinimapMain', self)
		map:setImageSource('/images/ui/premap')
		map:setId('mapId')
	  --  map:setOpacity(0.4)
		self:centerInPosition(map, {x= 995 , y=1251, z=7})
	end
	
	if self.cross and self.cross.pos.z ~= 7 then
		map:setVisible(false)
		self:setColor('black')
	else
		if self:getOpacity() == 1 then self:setColor('#1F007A') end
		map:setVisible(true)
	end 
end
...
function UIMinimap:addFlag(pos, icon, description)
  ...
  if not self:recursiveGetChildById('mapId') then
	local map = g_ui.createWidget('MinimapMain', self)
	map:setImageSource('/images/ui/premap')
	map:setId('mapId')
--	map:setOpacity(0.4)
	self:centerInPosition(map, {x = 995, y=1251, z=7})
  ...
  self:recursiveGetChildById('mapId'):insertChild(1, flag)
  flag.pos = pos
...
function UIMinimap:onZoomChange(zoom)
  for _,widget in pairs(self.alternatives) do
	...
	if map then
		...
		local mult = zoom
		local size = mult >= 0 and 1150 * math.pow(2, mult) or mult < 0 and 1150 / math.pow(2, (mult * -1))
		...
		for i = 1, number do
			local map = self:recursiveGetChildById('mapId' .. i)
			if map then
			if zoom <= -2 or self:getOpacity() == 1 then
			    map:setVisible(false)
			else
				map:setVisible(true)
			end
...
function UIMinimap:onMouseRelease(pos, button)
  ...
  self.fullmap = modules.game_minimap.getFullMapView()
 -- local mwindow = self:recursiveGetChildById('minimapWindow')

  if button == MouseLeftButton and (not self.fullmap or self.fullmap ~= 1)then
